...
export const useContentStore = defineStore('content', () => {
    ...
    const filterResources = (params) => {
        return resources.value.filter(resource => {
            ...
            if (params.type && resource.type !== params.type)
                return false;
            if (params.source && resource.source !== params.source)
                return false;
            if (params.skillType && resource.skillType !== params.skillType)
                return false;
            if (params.chapter && resource.chapter !== params.chapter)
                return false;
            if (params.category && resource.category !== params.category)
                return false;
            if (params.subCategory && resource.subCategory !== params.subCategory)
                return false;
            return true;
        });
    };
    ...
    const incrementViews = (resourceId) => {
        const resource = resources.value.find(r => r.id === resourceId);
        if (resource) {
            resource.views++;
        }
    };
    const toggleLike = (resourceId, userId) => {
        const resource = resources.value.find(r => r.id === resourceId);
        if (resource) {
