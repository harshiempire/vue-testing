...
export const useSubjectStore = defineStore("subject", () => {
    ...
    const averageClassScore = computed(() => {
        const allScores = students.value.flatMap((student) => Object.values(student.scores).filter((score) => score !== undefined));
        return allScores.reduce((sum, score) => sum + score, 0) / allScores.length;
    });
    const getStudentsByPerformance = () => {
        return {
            excellent: students.value.filter((student) => {
                const avgScore = Object.values(student.scores).reduce((sum, score) => sum + (score || 0), 0) / Object.values(student.scores).length;
                return avgScore >= 75;
            }),
    ...
    const addResource = (chapterId, resource) => {
        const chapter = chapters.value.find((c) => c.id === chapterId);
        if (chapter) {
