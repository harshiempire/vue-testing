...
export const useJourneyStore = defineStore('journey', () => {
    ...
    // Getters
    const getTimelineByDateRange = (startDate, endDate) => {
        return events.value.filter(event => event.date >= startDate && event.date <= endDate).sort((a, b) => b.date.localeCompare(a.date));
    };
    const getPortfolioByType = (studentId, type) => {
        return portfolios.value.filter(p => p.studentId === studentId && p.type === type);
    };
    const getAchievementsByType = (type) => {
        return achievements.value.filter(a => a.type === type);
    };
    // Actions
    const addPortfolioItem = (portfolioId, item) => {
    ...
    const verifyAchievement = (achievementId, status) => {
