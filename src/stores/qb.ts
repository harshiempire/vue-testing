...
export const useQBStore = defineStore('qb', () => {
  ...
  // Paper templates
  const paperTemplates = ref<PaperTemplate[]>([
    {
      id: '1',
  ...
  const questionPapers = ref<QuestionPaper[]>([]);

  const getQuestionsByChapter = (chapterId: string) => 
    questions.value.filter(q => q.chapterId === chapterId);

  const getQuestionsByType = (type: QuestionType) =>
    questions.value.filter(q => q.type === type);

  const getQuestionsByCognitiveLevel = (level: CognitiveLevel) =>
    questions.value.filter(q => q.cognitiveLevel === level);
  ...
  const updateQuestionPaper = (paper: QuestionPaper) => {
