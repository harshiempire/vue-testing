...
export const useAcademicsStore = defineStore('academics', () => {
    ...
    const pendingTasks = computed(() => tasks.value.filter(task => task.status === 'assigned'));
    const submittedTasks = computed(() => tasks.value.filter(task => task.status === 'submitted'));
    const reviewedTasks = computed(() => tasks.value.filter(task => task.status === 'reviewed'));
    // Add filter by category
    const filterTasks = (category) => {
        return tasks.value.filter(task => task.category === category);
    };
    const getTaskEvidence = (taskId) => evidence.value.filter(e => e.taskId === taskId);
    const getTaskReviews = (taskId) => reviews.value.filter(r => r.taskId === taskId);
    // Get self-assessment for a task
    const getTaskSelfAssessment = (taskId, studentId) => {
        return selfAssessments.value.find(sa => sa.taskId === taskId && sa.studentId === studentId);
    };
    // Approve/reject self-assessment
    const updateSelfAssessmentStatus = (assessmentId, status) => {
