...
export const usePTMStore = defineStore('ptm', () => {
    ...
    const inProgressSessions = computed(() => sessions.value.filter(session => session.status === 'scheduled' || session.status === 'in_progress'));
    ...
    const markParentAttendance = (sessionId, parent) => {
    ...
    const addTimeSlot = (date, startTime, endTime) => {
        const newSlot = {
            id: crypto.randomUUID(),
            teacherId: 'teacher1',
            date,
    ...
    const bookTimeSlot = (slotId, bookingData) => {
